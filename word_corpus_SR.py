"""
Corpus make 2.0 in TF2.0
制作带*的等长语料，长读和标签分别存放，行数相等
make the corpus according the absolute positions
build by python3.6 and tensorflow2.3-gpu env: tf2.3gpu
start with the calculation_positions()
and count the LR and the label bases in the absolute position
then cut the seq by pos, remove the char which is not ATGC，
save the source and target in two file
cue a sequence into several sub-sentences,each sub will lead by the seq name
cat the sub after decode in the infer phase
"""
"""
Run the file with "python --"
从pileup开始得到关于长读和标签的corpus，未对齐的先写到uncovered.fasta
最终生成long_corpus.txt和label_corpus.txt两个文件
"""
...
def short_absolute(pileup):
    ...
    for i in range(len(positions)):
        ...
        if temp != None: # 如果有删除，连带着数字出现，而非乱码起始字符，则统计删除出现的次数
            deletion_num = re.search(r"(?<=-)\d+", base_pileup).group(0)  # 找到删除的数目
            ...
            base_pileup = reencode_base_pileup(pileup[ref_position, 2], result,
                                               pileup_counts[i, 9])  # 改了reencode函数，使feature从长读本身来
            insertions, next_to_del = find_insertions(base_pileup)
            insertions_to_keep = []
            ...
            if not (np.any(pileup_counts[i, 0:9])):  # 如果没映射到ATGC上任意一个，则为N
                label_base = "N"
            else:
        ...
        labels = labels + label_base  # labels 与return的相对位置不能变
    return labels
...
def corpus(fasta_file, corpus_file, word_length, sentence_length):
    ...
    with open(fasta_file, mode='r') as reads:
        for read in reads.readlines():
            ...
            if read.startswith('>'):
                word = read.strip('\n')
                with open(corpus_file,mode='a') as corpus:
                    corpus.write(word + ' ')
            else:
                with open(corpus_file, mode='a') as corpus:
                    i = 0
                    for start in range(0, len(read) - word_length + 1, word_length):  # 最后一位为step设置为与window等长则二者无交叠，预测出的值直接拼接即可
                        end = start + word_length
                        word = read[start: end].strip('\n')
                        corpus.write(word)
                        i = i + 1
                        if i % sentence_length != 0:
                            corpus.write(' ') # 不到20末尾写空格，到20 写回车
                        else:
                            corpus.write('\n')  #  如果这句话有20个词那么换行，那就是有的有
                    if len(read) >= end:
                        start = len(read) - word_length
                        last_word = read[start: end]
...
def lr_label_build(mpileup_file):
    ...
    for i in range(0, len(pileup_total) - 1):
        if i == len(pileup_total) - 2:  # if the last seq
            sr = sr_cov[cur_start: i]
            cur_end = i
            if not (np.any(sr)):  # 如果没有短读覆盖则不进行处理
                read = ''.join([e for e in sequence[cur_start: cur_end]])
                write_wrs(seq_name[i], read, uncovered_file)  # 将序列写出去
                read = ''
            else:
                ...
                # write covered and long with absolute position reads out
                write_wrs(seq_name[i], long_read, covered_long)  # 也可以将label和long——read写出去。
                # write SR label with absolute position reads out
                write_wrs(seq_name[i], label, covered_label)  # 也可以将label和long——read写出去。
                seq_num += 1
        ...
        if not (np.any(sr)):  # 如果没有短读覆盖则不进行处理
            ...
            if len(long_read) == 0 or len(label) == 0:  # if the seq has no coverage from SR or truth,ignore it
                cur_start = i + 1
                seq_num += 1  # 如果长读没有任何覆盖，它的语料库该怎么设计？
                continue
...
def build_parser():
    ...
    parser.add_argument('--uncovered_file', type=str, help='Path of the filtered uncovered.fasta.')
    parser.add_argument('--covered_long', type=str, help='Path of the filtered covered_long.fasta.')
    parser.add_argument('--covered_label', type=str, help='Path of the filtered covered_label.fasta.')
...
if __name__ == '__main__':
    ...
    uncovered_file = data_dir + parsed_args.uncovered_file
    covered_long = data_dir + parsed_args.covered_long
    covered_label = data_dir + parsed_args.covered_label
    long_corpus = data_dir + parsed_args.long_corpus
    label_corpus = data_dir + parsed_args.label_corpus


    check_file(covered_label)
    check_file(uncovered_file)  # clear the output file to avoid overlying
    check_file(covered_long)  # clear the output file to avoid overlying
    check_file(long_corpus)
    check_file(label_corpus)
    lr_label_build(input_file)
    corpus(covered_long, long_corpus, word_length=word_length, sentence_length=sentence_length)
    corpus(covered_label, label_corpus,word_length=word_length, sentence_length=sentence_length)
